...
function agentIconHTML(id, label, size) {
...
function renderLocalAgents() {
  ...
  shown.forEach(cli => {
    ...
    let badge, onClick = null, disabled = false, title = null;
    if (retired) {
      badge = 'Retired';
      disabled = true;
...
function execShowModelControl(which) {
...
function renderCliModelSelect(id, force) {
...
function renderExecModel() {
  ...
  if (execMode === 'local') {
    ...
    const cli = localId ? (execLocal.clis || []).find(c => c.id === localId) : null;
...
function renderByokModelSelect(prov, force) {
  if (!execModelCli) return;
  ...
  execModelCli.setOptions(build((PROVIDER_MODELS[prov] || []).map(m => ({ id: m.v, label: m.l }))), saved);
  execModelCli.disabled = false;
...
function execSetModelHint(text, kind) {
...
function modelHintFor(res, prov) {
...
async function refreshExecMenu() {
  let status = null;
...
async function selectCloudAgent(prov) {
...
async function selectLocalCli(id) {
...
function openSettingsForProvider(prov) {
  closeExecMenu();
...
function openExecMenu() {
  ...
  _execDismiss = (e) => {
    ...
    if (e.target.closest && e.target.closest('.dd-list')) return;
    if (!execMenu.contains(e.target) && !execBtn.contains(e.target)) closeExecMenu();
  };
  _execEsc = (e) => {
