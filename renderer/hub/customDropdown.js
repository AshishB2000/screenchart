 ...
 */
(function () {
  let openDd = null; // at most one open at a time
  ...
  function makeDropdown(opts) {
    ...
    function onDocDown(e) {
      if (list.contains(e.target) || root.contains(e.target)) return;
      close();
    }
    ...
    Object.defineProperty(api, 'value', {
      get() { return value; },
      set(v) { value = v == null ? '' : String(v); buildList(); renderTrigger(); },
    });
    Object.defineProperty(api, 'disabled', {
      get() { return disabled; },
      set(v) { disabled = !!v; trigger.setAttribute('aria-disabled', String(disabled)); trigger.tabIndex = disabled ? -1 : 0; root.classList.toggle('dd-disabled', disabled); if (disabled) close(); },
    });
    Object.defineProperty(api, 'hidden', {
      get() { return root.hidden; },
      set(v) { root.hidden = !!v; if (v) close(); },
    });
    Object.defineProperty(api, 'placeholder', {
      get() { return placeholder; },
      set(v) { placeholder = v || ''; renderTrigger(); },
    });
    root._dd = api;
    renderTrigger();
    return api;
