...
function captureChartPNG(type, data, overrides) {
...
async function captureMapPNG(vizData, type) {
  ...
  try {
    ...
    holder.querySelectorAll('.cv-graph-controls').forEach(c => { c.style.display = 'none'; });
...
function tilesComplete(total, loaded) { return total > 0 && loaded >= total; }
function waitForMapTiles(holder, timeoutMs) {
  return new Promise((resolve) => {
...
function buildReportDoc({ title, analysis, headlineSegments, png }) {
  ...
  if (hasHeadline) {
    content.push({
      text: headlineSegments.map(s => (s && s.bold) ? { text: s.text, bold: true, color: C.strong } : (s ? s.text : '')),
  ...
  return {
    ...
    footer: (currentPage, pageCount) => {
      const cols = [{ text: 'Generated by Screenchart · ' + dateStr, style: 'footer' }];
...
async function exportPdf({ title, analysis, headlineSegments, type, png }) {
...
function buildReportPptx({ title, analysis, headlineSegments, png, logoPng }) {
  ...
  const footer = (slide, n) => slide.addText(`Screenchart  ·  ${dateStr}  ·  ${n}`,
  ...
  if (Array.isArray(headlineSegments) && headlineSegments.length) {
    s1.addText(
      headlineSegments.map(s => ({ text: (s && s.text) || '', options: { bold: !!(s && s.bold), color: (s && s.bold) ? C.strong : C.ink } })),
...
async function exportPptx({ title, analysis, headlineSegments, png }) {
...
function buildReportDocx({ title, analysis, headlineSegments, png, logoPng, pngDims }) {
  const d = window.docx;
  ...
  if (Array.isArray(headlineSegments) && headlineSegments.length) {
    children.push(new Paragraph({
      ...
      children: headlineSegments.map(s => new TextRun({
...
async function exportDocx({ title, analysis, headlineSegments, png }) {
...
async function exportPng({ title, analysis, headlineSegments, png }) {
...
function openExportDialog({ recommended, selectedExtra, current, vizData, entry, turnIdx, hasGeo, analysis, title, headlineSegments }) {
  closeExportDialog();
  recommended = recommended || [];
  if (!recommended.length) { showToast('No chart to export'); return; }
  const isMapType = (t) => t === 'map_bubble' || t === 'map_choropleth';
  const overridesFor = (t) => (entry && entry.chartOverrides && entry.chartOverrides[`${turnIdx}:${t}`]) || {};
  ...
  let fmtRow = null;
  const setFmtEnabled = (on) => {
    if (!fmtRow) return;
    fmtRow.querySelectorAll('.export-fmt-btn').forEach(b => { b.disabled = !on; b.classList.toggle('is-disabled', !on); });
  };

  function renderSelected(type, info) {
    if (!info.canRender) {
