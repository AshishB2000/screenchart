...
function destroyMapInContainer(container) {
...
async function loadChoroplethData(level) {
...
function renderGeoFallback(container, data, note) {
  ...
  const hasCoords = geo && geo.items.some(i => typeof i.lat === 'number' && typeof i.lng === 'number');
  if (hasCoords) {
...
function abbrevFor(item, featProps, level) {
  featProps = featProps || {};
  if (level === 'us_state') {
...
function buildPeriodGeo(labels, series, geoItems) {
  ...
  const itemsForPeriod = (idx) => geoItems.map((item, k) => {
    const arr = perItem[k];
...
function fillCentroidsFromBoundaries(items, geoData) {
...
async function renderMapInArea(container, data, type) {
  ...
  if (type === 'map_bubble' && geo.items.some(i => typeof i.lat !== 'number' || typeof i.lng !== 'number')) {
...
function _renderBubbleMap(map, wrap, geo, periodInfo, data) {
  const placeable = geo.items.filter(i => typeof i.lat === 'number' && typeof i.lng === 'number');
  const unplaceable = geo.items.filter(i => typeof i.lat !== 'number' || typeof i.lng !== 'number');
  ...
  else { const vs = placeable.map(i => i.value); minVal = Math.min(...vs); maxVal = Math.max(...vs); }
  ...
  let circleLayer = null, labelLayer = null, didFit = false;

  const itemsNow = () => {
    const base = usePeriods ? periodInfo.itemsForPeriod(periodIdx) : geo.items;
    return base.filter(i => typeof i.lat === 'number' && typeof i.lng === 'number');
  };
  ...
  function rebuildLabels() {
    ...
    const entries = itemsNow().filter(i => typeof i.value === 'number');
    const keys = valueLabelKeys(valueMode, [entries.map(e => e.value)]);
    const lg = L.layerGroup();
  ...
  drawCircles();
  _addUnmatchedNote(wrap, unplaceable.map(i => i.name));
...
function _renderChoroplethMap(map, wrap, geo, geoData, periodInfo, data) {
  const usePeriods = !!(periodInfo && periodInfo.periods && periodInfo.periods.length >= 2);
  let minVal, maxVal;
  if (periodInfo) { minVal = periodInfo.minVal; maxVal = periodInfo.maxVal; }   // fixed global scale
  else {
    const vs = geo.items.map(i => i.value).filter(v => typeof v === 'number');
  ...
  let periodIdx = usePeriods ? periodInfo.periods.length - 1 : 0;   // latest period
  let valueMode = 'maxmin';   // default: label the highest & lowest region
  let dataLayer = null, labelLayer = null, matchedCount = 0, didFit = false;
  ...
  const unmatchedNames = geo.items
    .filter(item => !matchedNames.has(normalizeName(item.name)))
    .map(item => item.name);
...
function _addUnmatchedNote(wrap, names) {
...
function _addMapPeriodDropdown(wrap, periods, defaultIdx, onChange) {
...
function _addMapValuesMenu(parent, getMode, onPick) {
...
function _addMapMenuButton(parent, data) {
  const btn = document.createElement('button');
  btn.type = 'button';
...
function _addBubbleLegend(wrap, minVal, maxVal, color, minR, maxR) {
...
function _addChoroplethLegend(wrap, minVal, maxVal) {
  const stops = document.documentElement.dataset.theme === 'dark'
    ? CHOROPLETH_STOPS_DARK : CHOROPLETH_STOPS_LIGHT;
  const leg = document.createElement('div');
  leg.className = 'cv-map-legend';
