...
function roundCoords(geom, decimals) {
  if (!geom) return geom;
  const d = typeof decimals === 'number' ? decimals : COORD_DECIMALS;
  const r = n => Math.round(n * 10 ** d) / 10 ** d;
  const rPair = p => [r(p[0]), r(p[1])];
  const rRing = ring => ring.map(rPair);
  switch (geom.type) {
    case 'Polygon':
      return { type: 'Polygon', coordinates: geom.coordinates.map(rRing) };
    case 'MultiPolygon':
      return { type: 'MultiPolygon', coordinates: geom.coordinates.map(poly => poly.map(rRing)) };
    case 'Point':
...
async function processSource(src) {
  ...
  try {
    ...
    console.warn(`  Warning: could not download ${src.name}: ${err.message}`);
  ...
  try {
    ...
    console.warn(`  Warning: could not parse ${src.name}: ${err.message}`);
    return;
  }
